...
# ╔═╡ 5a7d5023-4423-4921-903e-5384e9eb0c3b
...
This notebook is designed to make learning trigonometric functions both engaging and fun! We'll explore various trig functions and try to visualize their behavior using Pluto.jl and Julia's plotting capabilities. You'll find this notebook both informative and entertaining. Let's get started!

!!! question "This for you"
    This notebook is perfect for anyone with a basic understanding of trigonometry and an interest in seeing these functions through visualizations.

## Getting Started

We'll start by plotting the six fundamental trigonometric functions: sine, cosine, tangent, cosecant, secant, and cotangent.
...
Both sine and cosine functions are periodic with a period of 2π radians (or 360 degrees), meaning their values repeat in a predictable manner. They are foundational in fields such as geometry, physics, engineering, and signal processing, where they describe oscillatory phenomena, waveforms, and harmonic motion.

### Tangent 

The tangent function is known for its periodicity and rapid changes in value, especially near angles where the adjacent side approaches zero (resulting in vertical asymptotes). It helps determine angles and distances based on measurements or calculations involving right triangles.

### Cosecant, Secant, and Cotangent

Cosecant, secant, and cotangent are trigonometric functions that reciprocate sine, cosine, and tangent, respectively. Cosecant (csc θ) represents the ratio of the hypotenuse to the opposite side in a right triangle, given by 1/sin(θ). Secant (sec θ) shows the ratio of the hypotenuse to the adjacent side, defined as 1/cos(θ). Cotangent (cot θ) indicates the ratio of the adjacent side to the opposite side, expressed as 1/tan(θ). These functions play a crucial role in trigonometry, describing relationships in triangles and periodic behaviors in their graphs.
...
begin
	function frame_data(t)
	    ...
	    y_sec = sec.(x .+ phase)
	    y_cot = cot.(x .+ phase)
	    return y_sin, y_cos, y_tan, y_csc, y_sec, y_cot
...
ax1 = Axis(fig[1, 1], title="Sine and Cosecant Function")

# ╔═╡ 1b6f8670-8228-4af2-8cea-cb08341aa31a
ax2 = Axis(fig[2, 1], title="Cosine and Secant Function")

# ╔═╡ 0da00b20-5fbc-40f2-b5d0-602599b0f4b1
ax3 = Axis(fig[3, 1], title="Tangent and Cotangent Function")
...
# ╔═╡ 129af1c0-58d1-4552-bf80-3c28ad137a62
lines_sin = lines!(ax1, x, sin.(x), color=:blue)

# ╔═╡ 05e488c5-b177-4ad2-8da3-cd303f5269aa
lines_csc = lines!(ax1, x, csc.(x), color=:yellow)

# ╔═╡ 02db425f-069c-4b05-ba70-18e292e6404f
lines_cos = lines!(ax2, x, cos.(x), color=:red)

# ╔═╡ 5c5871d8-0560-4ab2-b1d1-d303baa33372
lines_sec = lines!(ax2, x, sec.(x), color=:orange)

# ╔═╡ 1bf7e7a5-03f9-484e-8d54-7fb82e4d9b0c
lines_tan = lines!(ax3, x, tan.(x), color=:green)

# ╔═╡ 2b4be877-e2ce-44e3-bc6d-b118da90a739
lines_cot = lines!(ax3, x, cot.(x), color=:brown)

# ╔═╡ 0aa9cce0-c89f-43e2-84a0-5f98682735de
function format_x_ticks(xticks)
    return ["0", "π/2", "π", "3π/2", "2π"]
end

# ╔═╡ 14d16c57-9d30-4e98-af9d-3829b684d070
ax1.xticks = (collect(0:π/2:2π), format_x_ticks(collect(0:π/2:2π)))

# ╔═╡ 2fe9b9e6-1365-402c-98d5-55e7afaa05a1
ax2.xticks = (collect(0:π/2:2π), format_x_ticks(collect(0:π/2:2π)))

# ╔═╡ 32f6fbac-5d6a-4b3c-96d2-1d1bb3adc598
ax3.xticks = (collect(0:π/2:2π), format_x_ticks(collect(0:π/2:2π)))

# ╔═╡ ddf4b35b-61ea-450e-bf26-24d1aadc7279
fig
...
line_hypotenuse = lines!(ax, [0, 1], [0, 0], color=:blue)

# ╔═╡ 73c6a3b4-0d4d-405d-b239-ea30e2b8f9d8
line_opposite = lines!(ax, [1, 1], [0, 1], color=:red)

# ╔═╡ f5ad03c1-e131-4dae-9a9b-ff5893dbd61e
line_adjacent = lines!(ax, [0, 1], [1, 1], color=:green)

# ╔═╡ b1069f0e-1acb-423a-9fcb-d3628493e811
angle_text = text!(ax, "θ", position=Point2(0.5, 0.5), align=(:center, :center))

# ╔═╡ 802e66fe-958c-47bb-b76f-a196189c3c6d
theta_value_text = text!(ax, "θ = 0", position=Point2(0.5, 0.5), align=(:center, :center), color=:black)

# ╔═╡ e0022f20-d4a5-4693-bc10-68dbc93b2bca
sin_text = text!(ax, "sin(θ)", position=Point2(1.1, 0.5), align=(:left, :center), color=:red)

# ╔═╡ af6f4ad2-d27d-44ed-a011-46fb66babda8
cos_text = text!(ax, "cos(θ)", position=Point2(0.5, -0.1), align=(:center, :top), color=:green)

# ╔═╡ 0de137c0-1446-48b1-a336-0b4657c39206
tan_text = text!(ax, "tan(θ)", position=Point2(1.1, 1.1), align=(:left, :top), color=:purple)

# ╔═╡ 0043b1a5-4718-4749-980c-094c861e1a7d
cot_text = text!(ax, "cot(θ)", position=Point2(-0.1, 1.1), align=(:right, :top), color=:brown)

# ╔═╡ 0afece51-b396-43bf-8081-d4852c56216a
sec_text = text!(ax, "sec(θ)", position=Point2(1.1, 0.0), align=(:left, :center), color=:orange)

# ╔═╡ 911a0a09-f47f-4af8-9717-3384c879fc0b
csc_text = text!(ax, "csc(θ)", position=Point2(0.0, 1.1), align=(:center, :bottom), color=:blue)



# ╔═╡ f5f41ead-a22d-4b30-9c2c-920bc153d195
function update_rotating_triangle!(t)
    angle = 2π * t / frames
    cos_angle = cos(angle)
    sin_angle = sin(angle)
    tan_angle = tan(angle)
    ...
    line_hypotenuse[1] = [Point2(0, 0), Point2(cos_angle, sin_angle)]
    line_opposite[1] = [Point2(cos_angle, 0), Point2(cos_angle, sin_angle)]
    line_adjacent[1] = [Point2(0, sin_angle), Point2(cos_angle, sin_angle)]

    angle_text.position = Point2(cos_angle / 2, sin_angle / 2)
    sin_text.position = Point2(cos_angle + 0.1, sin_angle / 2)
    cos_text.position = Point2(cos_angle / 2, -0.1)

    tan_text.position = Point2(cos_angle + 0.1, sin_angle + 0.1)
    cot_text.position = Point2(-0.1, sin_angle + 0.1)
    sec_text.position = Point2(cos_angle + 0.1, 0.0)
    csc_text.position = Point2(0.0, sin_angle + 0.1)

    tan_text.text = "tan(θ) = " * string(round(tan_angle, digits=2))
    cot_text.text = "cot(θ) = " * string(round(cot_angle, digits=2))
    sec_text.text = "sec(θ) = " * string(round(sec_angle, digits=2))
    csc_text.text = "csc(θ) = " * string(round(csc_angle, digits=2))
end
...
# ╔═╡ a650837f-2f30-495b-874f-203637c41ef5


# ╔═╡ ae4caa40-a60a-4757-839d-e3f687729e74
md"### Interactive Rotating Unit Circle"

# ╔═╡ 26534ba0-0237-4a05-bd9f-684c8cc1ba5d
gr()  # Set the plotting backend (e.g., GR)

# Function to plot the unit circle and a triangle at angle theta (in degrees)

# ╔═╡ ec513f79-c42b-44b4-b423-a59082f00c47
function plot_unit_circle(theta_degrees)
    theta = deg2rad(theta_degrees)  # Convert degrees to radians
    r = 1.0
    
    # Calculate triangle points based on theta
    x1, y1 = 0.0, 0.0
    x2, y2 = r * cos(theta), r * sin(theta)
    x3, y3 = r * cos(theta + π / 2), r * sin(theta + π / 2)
    
    # Create plot
    circle_plot = Plots.plot(legend=false, aspect_ratio=:equal, xlims=(-1.5, 1.5), ylims=(-1.5, 1.5))
    Plots.plot!(circle_plot, cos, sin, 0, 2π, color=:black, linewidth=2, label="Unit Circle")
    Plots.plot!(circle_plot, [x1, x2, x3, x1], [y1, y2, y3, y1], color=:blue, alpha=0.5, linewidth=2, label="Triangle")
    
    return circle_plot
end

# ╔═╡ 7e737b87-a9e9-4e74-972e-4b296155824b
function trig_values(theta_degrees)
...
# ╔═╡ e9f60a44-e891-4688-8f16-95bc12d675af
md"### Degree Slider"

# ╔═╡ 226bf6f0-2ef2-469b-b9e8-53ba7aa63f04
@bind theta_degrees PlutoUI.Slider(0:1:360, show_value=true, default=0)

# ╔═╡ 5fbdf4ef-126a-46e7-b1af-748e4fdd2b07
md"### Unit Circle with Triangle"

# ╔═╡ 2e8cd2d3-0653-4105-9fac-c47e56f1d975
circle_plot = plot_unit_circle(theta_degrees)

# ╔═╡ 92042613-c7e9-4815-a9ab-c53ab467eeff
sine, cosine, tangent = trig_values(theta_degrees)

# ╔═╡ 0f7ee141-60f5-4272-a8e9-974739be8ca5
md"""
### Trigonometric Functions at θ = theta_degrees°:
- **sin(θ)** = $sine
- **cos(θ)** = $cosine
- **tan(θ)** = $tangent
"""
...
manifest_format = "2.0"
project_hash = "1bf967ef513fb531361d29ddde34546a3dffc60f"
