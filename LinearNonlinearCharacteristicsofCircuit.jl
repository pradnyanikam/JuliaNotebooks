...
# ╔═╡ 39740eba-8123-45a4-8d18-a19402dea0cc
begin
	using Plots
	using Images
	md"""
	!!! info "Ploting Packages"
		...
		[ImageShow](https://www.juliapackages.com/p/Images): Enhances the displaying of the images in the interactive visualization and the gallery.
	"""
end

# ╔═╡ b1c303f4-2979-46e5-8ea2-25b253805a06
begin
...
# ╔═╡ f7111fff-3cf2-42f3-8847-073704d320de
md"""
# 💡 Electronics: The Foundation of Technology 💡
"""

# ╔═╡ e19cb057-dda3-4515-897e-f1a17cceabd6
TableOfContents()

# ╔═╡ 84383f69-733d-47ab-8078-90b727c50382
html"""
<div>
...
!!! information
	...
	**In this, we will learn some important basic concepts.**
	
	- **Components**: Electronics circuits are made up of various components such as resistors, capacitors, transistors, and integrated circuits.
	- **Functionality**: These components work together to perform various functions like amplification, filtering, and signal processing.
	- **Applications**: Electronics circuits are used in a wide range of applications including communication systems, computing devices, and consumer electronics.
	- **Importance**: Understanding the basic concepts of electronics is important for anyone interested in technology and innovation.
"""

# ╔═╡ 122c2c7a-2200-4de0-ad59-0ee083cd016f
md"""## Understanding Linear and Nonlinear Circuit Characteristics"""
...
Diodes are electronic components that allow current to flow in one direction only. They're used in circuits to control the direction of electrical currents. When a diode is forward-biased (voltage applied in the correct direction), it allows current to flow; when reverse-biased (voltage applied in the opposite direction), it blocks current. Diodes have a forward voltage drop, which is the voltage required to make them conduct. Different types of diodes, like LEDs and rectifier diodes, have specific applications based on their unique characteristics.
...
md"""
## Conclusion
Electronics circuits are the invisible architects of our modern world, quietly enabling the devices and systems we rely on every day. They are the foundation of communication networks, the brains behind our smart devices, and the powerhouses driving our industries. Without electronics circuits, the digital age as we know it would not exist, highlighting their fundamental importance in shaping the technological landscape of today and tomorrow"""

# ╔═╡ c7a9e1a4-10d5-4639-bc6c-deac83f4d6f2
md"## Packages"
...
""" 	
Calculates the voltage across a resistor using Ohm's Law, given the current through the resistor and the resistance.
	
# Arguments
	
- `I::Number`: The current through the resistor in amperes (A).
- `R::Number`: The resistance of the resistor in ohms (Ω).
	
# Returns
	
- `Number`: The voltage across the resistor in volts (V).
	
# Examples
	
julia> ohms_law_voltage(2, 5)
10
"""
function ohms_law_voltage(I, R)
		return I * R
	end

# ╔═╡ a2d92ee1-846a-4a68-b321-a20d15e56fc6
"""
Calculates the current through a resistor using Ohm's Law, given the voltage across the resistor and the resistance.

# Arguments

- `V::Number`: The voltage across the resistor in volts (V).
- `R::Number`: The resistance of the resistor in ohms (Ω).

# Returns

- `Number`: The current through the resistor in amperes (A).

# Examples
julia> ohms_law_current(10, 5)
2.0
"""
function ohms_law_current(V, R)
    return V / R
end

# ╔═╡ 0edb3244-cd27-4dd1-9497-21fccf7b2617
"""
Calculates the current through a diode using the Shockley diode equation, given the voltage across the diode, the saturation current, the ideality factor, and the thermal voltage.

# Arguments

- `V::Number`: The voltage across the diode in volts (V).
- `I_s::Number`: The saturation current of the diode in amperes (A).
- `n::Number`: The ideality factor of the diode (unitless).
- `V_t::Number`: The thermal voltage in volts (V).

# Returns

- `Number`: The current through the diode in amperes (A).

# Examples
julia> diode_current(0.7, 1e-12, 1, 0.025)
0.00030965645142092824"""
function diode_current(V, I_s, n, V_t)
    return I_s * (exp(V / (n * V_t)) - 1)
end

# ╔═╡ 2f357d29-52ef-47ab-aa6e-cee99c7f4014
"""
Calculates the voltage across a diode using a modified form of the Shockley diode equation, given the current through the diode, the saturation current, the ideality factor, and the thermal voltage. This function handles both forward and reverse currents.

# Arguments

- `I::Number`: The current through the diode in amperes (A).
- `I_s::Number`: The saturation current of the diode in amperes (A).
- `n::Number`: The ideality factor of the diode (unitless).
- `V_t::Number`: The thermal voltage in volts (V).
...
- `Number`: The voltage across the diode in volts (V).

# Examples
julia> diode_voltage(0.00030965645142092824, 1e-12, 1, 0.025)
0.7
"""
function diode_voltage(I, I_s, n, V_t)
    if I < 0
        return V_t * log(1 + abs(I / I_s)) * n
    else
        return V_t * log(1 + (I / I_s)) * n
    end
end

# ╔═╡ 10f999a6-95c8-44df-9ca3-e19c1092a586
md"""## Interactivity extensions
###### This is Custumize functions defined to handle sliders.
"""
...
Non_linear_Slider = @bind NonLinearparameters format_sliderParameter(
			title = "Electrical Parameters:", 
			[
				SliderParameter(
					lb = -10.0,
...
# ╔═╡ 2b606fbf-cfcf-4aec-bbbc-b108aa1a22c0
md"""## CSS Styles

###### This is the CSS style to handle slider positions besides of the graph 


"""

# ╔═╡ c9090da2-fb24-43c9-8f7c-989bedbe0831
begin
...
# ╔═╡ 5d7b7ff1-cb2c-4b6a-94ae-a5c63b7f9a8c
begin
	resistance_slider = viz_parameters[:resistance_slider]
	voltage_slider = viz_parameters[:voltage_slider]
	current_slider = viz_parameters[:current_slider]
end;

# ╔═╡ 663050e4-9693-460b-a93d-5f43b7409e4e
begin
	Nonlinearvoltage_slider = NonLinearparameters[:voltage_slider_diode]
end

# ╔═╡ de5ea53c-3e27-4dbc-9fb0-5b2e9d1870ab
begin
	voltage_range_forward = 0:0.1:Nonlinearvoltage_slider  # Forward bias range
	voltage_range_reverse = -Nonlinearvoltage_slider:0.1:0  # Reverse bias range
	
end;

# ╔═╡ 2b2c6678-709f-40b0-a7cf-96a92f20f33b
voltage_range = 0:0.1:voltage_slider  # voltage range

# ╔═╡ d469c0ee-946f-4799-8fa5-caeb1a2e4d00
currents = ohms_law_current(voltage_range,resistance_slider)

# ╔═╡ 6f9e86a5-8f60-44c6-86bd-bcfb709132c0
current_range = 0:0.1:current_slider

# ╔═╡ 49ce7603-d110-4d60-a347-315fe4d72fa8
voltages= ohms_law_voltage(current_range,resistance_slider)
...
# ╔═╡ 03ee7f8e-dbf9-466b-8845-02ca0a0e776d
begin
	currents_forward = diode_current.(voltage_range_forward, I_s, n, V_t)
	currents_reverse = diode_current.(voltage_range_reverse, I_s, n, V_t)
end;
...
manifest_format = "2.0"
project_hash = "6f360119bb094985270bf75c23d889ab5a43a7ea"
