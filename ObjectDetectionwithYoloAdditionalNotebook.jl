...
# ╔═╡ 0b1b1bf8-525b-474c-a413-98589b11f66e
begin
	using Plots
	using Images
	using Flux
    ...
    using ImageTransformations
    using ImageEdgeDetection, MosaicViews
    using TestImages, Colors, FileIO
...
# ╔═╡ 26c3e136-2396-4085-8aa3-7a909f558cbb
begin
...
md"""
# 📷 Real-Time Object Detection and Image Filtering with YOLOv3
"""

# ╔═╡ a87c71a0-67e7-41d0-a219-da4bd63b6c0a
md""" 
!!! information
	**Welcome to this interactive Pluto.jl notebook! In this notebook, we utilize the power of YOLOv3 for real-time object detection directly from your webcam.** 
"""
...
- **Live Webcam Input**: Capture live images from your webcam.
- **Object Detection**: Detect multiple objects in real-time using YOLOv3.
- **Image Filters**: Apply various filters to enhance your images:
  - **Blur**: Smooths the image to reduce noise.
  - **Contrast Adjustment**: Increase or decrease the contrast.
...
By integrating object detection and image filtering, this notebook demonstrates a practical approach to enhancing image analysis tasks, useful in applications ranging from security systems to autonomous vehicles.
...
# ╔═╡ 382b7605-dc06-4048-a58e-def4a01d2c2d
@bind captureMode Select(["Image","Live Cam"])

# ╔═╡ e09fd55c-9863-48ef-9876-31318ab45b7e
begin
	if(captureMode=="Live Cam")
		@bind img_raw PlutoUI.WebcamInput(help=false, avoid_allocs=true, max_size=600)
	else
		println("Putting Cameraman image as a example")
		img_raw = testimage("cameraman")
		img_raw = Images.imresize(img_raw, 600, 600)

	end
end

# ╔═╡ 796f02c2-486b-4875-b840-2178273c29bb
md"### Apply Edge Detection? $(@bind apply_edge_detection CheckBox(default=false))"

# ╔═╡ d5812f6d-ad2e-4e00-821d-0c443d6812a3
md"## User values"

# ╔═╡ 3080ed33-1c0f-4246-a73f-2590dbcfa7c7
md"## Functions"

# ╔═╡ da7f3d3e-666e-4d20-9398-fbea70947498
md"""
    apply_filters(img, blur_radius, contrast_adj, apply_edge_detection) ->image array
Apply the filters given by the user.

##### Args

- `img::Number`: given images.
- `blur_radius::Number`: Blur radius value.
- `contrast_adj::Number`: contrast value.
- `apply_edge_detection::bool`: Tapply edge detection true or false.

##### Returns

- `Imagefiltered`: Image with the filtered applied

"""

# ╔═╡ 40126d87-ff83-4861-82f4-255c783614c3
function apply_filters(img, blur_radius, contrast_adj, apply_edge_detection)
        # Apply blur if blur_radius is greater than 0
	if blur_radius > 0
		img = imfilter(img, Kernel.gaussian(blur_radius))
	end

	# Adjust contrast
	if contrast_adj != 0
		alg = Equalization(nbins = contrast_adj)
	...
	if apply_edge_detection
		...
		img = mosaicview(img, img_edges₁, img_edges₂, img_edges₃; nrow = 2)
	end
	
	return img
end;

# ╔═╡ 9e0c20e6-bdcc-43f7-8b45-2c3b7d22d111
md"""
    prepareImageForYOLO(img, yolomod) ->image array
Apply the filters given by the user.

##### Args

- `img::Number`: given images.
- `yolomod::`: yolomod object.

##### Returns

- `imagewithpadding`: Image with the yolo functions applied

"""

# ╔═╡ 6dc214be-8d80-4e8a-a742-6d259d93cc8b
function prepareImageForYOLO(img, yolomod)
	img_resized = imresize(img, ())  # Adjust to the correct size if needed
	img_padded, padding = prepareImage(img_resized, yolomod)
	return img_padded, padding
end

# ╔═╡ d7a5860d-b491-4ef8-9f21-2525a7989590
md"""
   objectDetection(img_raw,detect_thresh,overlap_threshold) ->image with boxes
Apply the object detection parameters and thresholds.Get the detected object image.

##### Args

- `img_raw::Number`: given images.
- `detect_thresh::Number`: detection threshold.
- `overlap_threshold::Number`: Overlap threshold.

##### Returns

- `Imagewithboxesaroundobjects`: Image with boxes around object

"""

# ╔═╡ 2a76107c-42ef-4a1b-86ae-d36a90638cea
function objectDetection(img_raw,detect_thresh,overlap_threshold)
    yolomod = YOLO.v3_608_COCO(batch=1, silent=true)

	batch = emptybatch(yolomod)  # Create a batch object
	img_padded, padding = prepareImageForYOLO(img_raw, yolomod)  # Prepare image

	batch[:,:,:,1] = img_padded  # Send resized image to the batch

	# Run the model
	res = yolomod(batch, detectThresh=detect_thresh, overlapThresh=overlap_threshold)

	# Draw boxes on the image
	imgBoxes = drawBoxes(img_raw, yolomod, padding, res)

	save("result.png", imgBoxes)  # Save the image (optional)
	imgBoxes
	return imgBoxes
end

# ╔═╡ b7d87b24-b3f6-409c-9154-47b15567b6d8
md"## Interactivity extensions"

# ╔═╡ f9e17498-6d2b-4036-a35b-b7a20f7e7628
md""" ## Custom Types"""

# ╔═╡ 1443b010-cd56-46bc-bfb7-2fb58b1c7ef0
...
# ╔═╡ 66972708-7554-4d13-82c5-9d7cfd85f5c8
...
# ╔═╡ ef230c78-f9c7-4073-9dee-2740c0453b23
...
# ╔═╡ b3102d63-151e-44eb-bdc3-4e18240c2a60
Image_sliders = @bind Image_parameters format_sliderParameter(
...
# ╔═╡ f9986481-34ef-47f3-ad3d-18115b3331e3
begin
	blur_radius = Image_parameters[:blur_radius]
	contrast_adj = Image_parameters[:contrast_adj]
end

# ╔═╡ 5f3cc2c3-728e-42cb-8670-4e065611bf81
object_Sliders = @bind Object_parameters format_sliderParameter(
			title = "Object Detection Parameters", 
			[
				SliderParameter(
					lb = 0.0,
					ub = 1.0, 
					default = 0.8, 
			 		step = 0.1, 
					alias = :overlap_threshold, 
					label = "overlap_threshold"
				),
				SliderParameter(
					lb = 0.0,
					ub = 1.0, 
...
# ╔═╡ 13351fbe-7426-4ce3-be1f-af7695dd7067
begin
...
# ╔═╡ 4b74cdc2-804f-440a-8f69-11cf45bb725b
begin
	overlap_threshold = Object_parameters[:overlap_threshold]
	detect_thresh = Object_parameters[:detect_thresh]
end

# ╔═╡ a1ef841e-5746-4432-9c0a-c4c09f8b30d5
if img_raw !== nothing
	if mode == "Object Detection"
		image_data=objectDetection(img_raw,detect_thresh,overlap_threshold)
	else
		image_data = apply_filters(img_raw, blur_radius, contrast_adj, apply_edge_detection)
	end
	image_data
else
	"Waiting for webcam input..."
end

# ╔═╡ 19a0343e-8774-464a-a00e-164ae3a3cadd
md"## CSS Styles"

# ╔═╡ 7bd8e454-7658-4dde-bc98-4fb14ba9121e
@htl("""
...
ImageInpainting = "2bcd8d84-2ef9-5317-bfa0-0ed5605635ad"
...
ImageInpainting = "~1.0.0"
ImageTransformations = "~0.10.1"
Images = "~0.26.1"
Metalhead = "~0.9.3"
MosaicViews = "~0.3.4"
ObjectDetector = "~0.3.1"
Parameters = "~0.12.3"
Plots = "~1.40.8"
PlutoUI = "~0.7.60"
TestImages = "~1.8.0"
...
manifest_format = "2.0"
project_hash = "5c52bcee801f455782cd2bbaea5ac764ca279c9f"
...
[[deps.ImageInpainting]]
deps = ["CpuId", "FFTW", "ImageFiltering", "ImageMorphology"]
git-tree-sha1 = "8b3749ddbd1d4ec86236fe4835f894e2749a157a"
...
[[deps.ObjectDetector]]
deps = ["BenchmarkTools", "Flux", "ImageCore", "ImageDraw", "ImageFiltering", "ImageTransformations", "LazyArtifacts", "Pkg", "PrecompileTools", "PrettyTables"]
git-tree-sha1 = "889655fcccbd139c8d1bbf61e4ab0207dbbac10d"
uuid = "3dfc1049-5314-49cf-8447-288dfd02f9fb"
version = "0.3.1"

    [deps.ObjectDetector.extensions]
    CUDAExt = ["CUDA", "cuDNN"]

    [deps.ObjectDetector.weakdeps]
    CUDA = "052768ef-5323-5732-b1bb-66c8b64840ba"
    cuDNN = "02a925ec-e4fe-4b08-9a7e-0d78e3d38ccd"
